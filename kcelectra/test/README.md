...
> 27개 테스트 문장으로 평가한 결과 (Baseline, E1, E2, E4)
...
1. **E4 성능 대폭 향상**: Accuracy 48.1% → 66.7% (+18.6%p)
2. **하드카운터 개선**: 특수문자/짧은 문장 33% → 67% (2배)
3. **일반 욕설 개선**: 40% → 80% (2배)
4. **FP/FN 동시 감소**: FP 6→4, FN 8→5

### 남은 문제점

1. **변형욕설 부족**: 43%로 아직 낮음 → E5에서 가중치 조정
2. **하드네거티브**: 75%로 개선 안됨 (쌈@뽕 등)

### 개선 방향 (E5)

- **E5**: E4 + 가중치 반영 (정상:악성 = 4:1) → FP 추가 감소 기대
- **Threshold 튜닝**: E5 결과 후 0.7 기준으로 재조정
